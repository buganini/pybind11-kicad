...
if ! "$ROOT_DIR/scripts/run.sh" python -c "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('pytest') else 1)"; then
    ...
    echo "  $ROOT_DIR/scripts/run.sh python -m pip install -r $ROOT_DIR/compat/pcbnewswig-test-requirements.txt" >&2
    exit 127
fi

cd "$TEST_DIR"
exec "$ROOT_DIR/scripts/run.sh" python -m pytest "$TEST_PATH" "$@"
